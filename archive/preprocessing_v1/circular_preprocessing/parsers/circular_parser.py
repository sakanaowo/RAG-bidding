...
class CircularParser:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            # Main sections
            "chapter": (r"^(CHƯƠNG|Chương)\s+([IVXLCDM]+|\d+)[\.\:]?\s*(.*)$", 2),
            "section": (r"^(MỤC|Mục)\s+(\d+)[\.\:]?\s*(.*)$", 3),
            # Content structure
            "regulation": (r"^(QUY ĐỊNH|Quy định)\s+(\d+)[\.\:]?\s*(.*)$", 4),
            "guidance": (r"^(HƯỚNG DẪN|Hướng dẫn)\s+(\d+)[\.\:]?\s*(.*)$", 4),
            "article": (r"^Điều\s+(\d+[a-z]?)[\.\:]?\s*(.*)$", 4),
            "clause": (r"^(\d+)[\.\:]?\s+(.+)$", 5),
            "point": (r"^([a-zđ])\)\s*(.+)$", 6),
            "sub_point": (r"^-\s*(.+)$", 7),
        }
    ...
    def _parse_line(self, line: str, line_num: int) -> Optional[StructureNode]:
        ...
        if pattern_name == "circular_header":
            ...
            return StructureNode(
                ...
                title=title.strip(),
                level=level,
                metadata={"line_number": line_num, "raw_text": line},
            )
    ...
    def _find_parent(
        self, current_path: List[StructureNode], level: int
    ) -> StructureNode:
            ...
            node.metadata.update(
                {
                    ...
                    "content_length": (
                        len(node.text_content) if node.text_content else 0
                    ),
                }
            )
