...
def test_memory_stability():
    ...
    # Sample documents
    docs = [
        Document(page_content="Luật Đấu thầu 2023 quy định về quy trình đấu thầu.", metadata={}),
        Document(page_content="Nghị định 63/2014 hướng dẫn Luật Đấu thầu.", metadata={}),
    ...
    gpu_growth_after_warmup = gpu_memories[-1] - gpu_memories[2] if len(gpu_memories) > 2 else 0
    
    print(f"Growth after warmup (iter 20-100): RAM {mem_growth_after_warmup:.1f}MB, GPU {gpu_growth_after_warmup:.1f}MB")
    
    # After warmup, memory should be stable (<20MB growth)
    if mem_growth_after_warmup < 20 and gpu_growth_after_warmup < 20:
        print(f"✅ PASS: Memory stable after warmup (RAM +{mem_growth_after_warmup:.1f}MB, GPU +{gpu_growth_after_warmup:.1f}MB)")
        return True
    else:
        print(f"❌ FAIL: Memory leak detected! (RAM +{mem_growth_after_warmup:.1f}MB, GPU +{gpu_growth_after_warmup:.1f}MB)")
        return False
...
def test_performance_consistency():
    ...
    if std_dev < avg_latency * 0.3:
        print(f"✅ PASS: Performance consistent (std dev {std_dev/avg_latency*100:.1f}% of mean)")
        return True
    else:
        print(f"❌ FAIL: Performance unstable (std dev {std_dev/avg_latency*100:.1f}% of mean)")
        return False
