...
def test_circular_pipeline():
    ...
    try:
        ...
        print(
            ...
            if isinstance(orig_chars, int)
            else f"   Original chars: {orig_chars}"
        )
        print(
            f"   Cleaned chars: {clean_chars:,}"
            if isinstance(clean_chars, int)
            else f"   Cleaned chars: {clean_chars}"
        )
        print(f"   Structure nodes: {stats.get('total_nodes', 'N/A')}")
        print(f"   Chunks: {stats.get('total_chunks', len(results['chunks']))}")
        print(
            f"   Avg chunk size: {avg_chunk:.0f}"
            if isinstance(avg_chunk, (int, float))
            else f"   Avg chunk size: {avg_chunk}"
        )
        ...
        if "integrity_report" in results:
            ...
            print(
                f"   Checks: {integrity.passed_checks}/{integrity.total_checks} passed"
            )
