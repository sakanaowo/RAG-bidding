...
def test_bge_threshold(test_pairs: List[TestPair], reranker) -> Dict:
    ...
    # Statistics
    print(f"\n📊 Statistics:")
    print(f"   Similar:   min={min(similar_scores):.4f}, max={max(similar_scores):.4f}, mean={np.mean(similar_scores):.4f}")
    print(f"   Different: min={min(different_scores):.4f}, max={max(different_scores):.4f}, mean={np.mean(different_scores):.4f}")
    ...
    print(f"   Separation:   {separation:.4f} {'✅ Clear separation!' if separation > 0 else '❌ Overlap detected'}")
    ...
    for t in np.arange(0.0, 1.01, 0.05):
        ...
        print(f"      {t:.2f}   |   {similar_hit:2d}/{len(similar_scores)}     |     {different_fp}/{len(different_scores)}      |  {accuracy:.1%}{marker}")
    
    print(f"\n🎯 RECOMMENDED BGE THRESHOLD: {best_threshold:.2f}")
    print(f"   Accuracy: {best_accuracy:.1%}")
    print(f"   Similar hit rate: {sum(1 for s in similar_scores if s >= best_threshold)}/{len(similar_scores)}")
    print(f"   False positive rate: {sum(1 for s in different_scores if s >= best_threshold)}/{len(different_scores)}")
...
def test_speed_comparison(embedder, reranker, num_cached_queries: int = 100) -> Dict:
    ...
    # Test query
    test_query = "Điều kiện về năng lực tài chính khi tham gia đấu thầu yêu cầu những gì"
    ...
    print(f"   Cosine scan time: {cosine_time_ms:.2f}ms for {num_cached_queries} queries")
    ...
    print(f"   Cosine pre-filter: {len(candidates)} candidates in {cosine_filter_time_ms:.2f}ms")
    
    # Step 2: BGE rerank
    if candidates:
        pairs = [[test_query, cached_queries[i] if i < len(cached_queries) else f"Query {i}"] 
        ...
        print(f"   BGE rerank time: {bge_time_ms:.1f}ms for {len(candidates)} candidates")
    ...
    print(f"   Overhead:          {total_v2 - (embed_time_ms + cosine_time_ms):.1f}ms (BGE rerank)")
    ...
    for size in [100, 500, 1000, 5000, 10000]:
        ...
        print(f"   {size:6d}    |  {v1_total:8.1f}ms |  {v2_total:8.1f}ms |  +{overhead:.1f}ms ({overhead_pct:.1f}%)")
...
def test_accuracy_simulation(reranker, test_pairs: List[TestPair]) -> Dict:
    ...
    for pair in similar_pairs:
        ...
        if best_match:
            ...
            bge_score = float(reranker.model.predict(bge_pairs, show_progress_bar=False)[0])
        ...
        print(f"   {pair.description[:25]:25s} | {v1_marker} {best_cosine:.4f}         | {v2_marker} {bge_score:.4f}")
    
    print(f"\n📊 Accuracy Results:")
    print(f"   V1 (Cosine≥{cosine_threshold_v1}): {v1_hits}/{len(similar_pairs)} = {v1_hits/len(similar_pairs):.1%}")
    print(f"   V2 (BGE≥{bge_threshold_v2}):    {v2_hits}/{len(similar_pairs)} = {v2_hits/len(similar_pairs):.1%}")
