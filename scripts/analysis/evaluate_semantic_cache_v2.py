...
TEST_PAIRS: List[TestPair] = [
    ...
    TestPair(
        ...
        "Điều kiện hủy bỏ cuộc đấu thầu",
        "similar"
    ),
    TestPair(
        "Điều kiện tham gia đấu thầu",
        "Yêu cầu để được tham dự đấu thầu",
        "similar"
    ),
    TestPair(
...
def run_evaluation(bge_threshold: float = 0.85, verbose: bool = True):
    ...
    for i, pair in enumerate(TEST_PAIRS, 1):
        ...
        if verbose:
            ...
            match_str = f"MATCH (bge={result.bge_score:.4f})" if result.matched else "NO MATCH"
            expected = "should match" if pair.should_match else "should NOT match"
            
            print(f"Pair {i:2d} [{pair.category.upper():9s}] {status} {match_str:30s} ({expected})")
    ...
    recall = (similar_correct + identical_correct) / (len(similar_results) + len(identical_results)) * 100
    
    if verbose:
        print(f"\n📊 SIMILAR QUERIES (Recall):")
        print(f"   - Similar matched: {similar_correct}/{len(similar_results)} = {similar_correct/len(similar_results)*100:.1f}%")
        print(f"   - Identical matched: {identical_correct}/{len(identical_results)} = {identical_correct/len(identical_results)*100:.1f}%")
    ...
    false_positive_rate = (len(different_results) - different_correct) / len(different_results) * 100
    
    if verbose:
        print(f"\n📊 DIFFERENT TOPICS (Precision):")
        print(f"   - Correctly rejected: {different_correct}/{len(different_results)} = {different_correct/len(different_results)*100:.1f}%")
...
def run_threshold_sweep():
    ...
    for threshold in thresholds:
        ...
        if result:
            results.append(result)
            print(f"Accuracy={result['accuracy']:.1f}%, Recall={result['recall']:.1f}%, Precision={result['precision']:.1f}%")
    ...
    print("=" * 80)
    print()
    print(f"{'Threshold':<10} {'Accuracy':<12} {'Recall':<12} {'Precision':<12} {'Failed':<8}")
    print("-" * 60)
    ...
    for r in results:
        print(f"{r['bge_threshold']:<10.2f} {r['accuracy']:<12.1f} {r['recall']:<12.1f} {r['precision']:<12.1f} {r['failed_count']:<8}")
