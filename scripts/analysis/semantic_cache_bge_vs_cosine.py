...
def get_test_pairs() -> List[TestPair]:
    ...
    return [
        ...
        TestPair(
            ...
            "similar",
            "Năng lực tài chính - original case",
        ),
        TestPair(
        ...
        TestPair(
            ...
            "different",
            "Nhà thầu chính vs nhà thầu phụ",
        ),
        TestPair(
            "Yêu cầu về năng lực tài chính của nhà thầu",
            "Quy trình mở thầu diễn ra như thế nào",
            "different",
            "Topics hoàn toàn khác nhau",
        ),
        # =====================================================================
...
def evaluate_openai_cosine(
    test_pairs: List[TestPair], embedder: OpenAIEmbedder
...
def evaluate_bge_crossencoder(
    test_pairs: List[TestPair], reranker
) -> Tuple[List[float], List[float], List[float], float]:
    """
...
def analyze_threshold(similar: List[float], different: List[float], method_name: str):
    ...
    if has_clear_separation:
        ...
        all_scores = [(s, "similar") for s in similar] + [
            (s, "different") for s in different
        ]
        ...
        for threshold in np.arange(min(similar) - 0.1, max(different) + 0.1, 0.01):
            ...
            accuracy = (similar_caught + different_rejected) / (
        ...
        print(
            f"     Best threshold: {best_threshold:.4f} (accuracy: {best_accuracy:.1%})"
        )
        ...
        print(
            f"     At {best_threshold:.4f}: {similar_caught}/{len(similar)} similar caught, {false_positives}/{len(different)} false positives"
        )
...
def main():
    ...
    cosine_similar, cosine_different, cosine_identical, cosine_time = (
        evaluate_openai_cosine(test_pairs, embedder)
    )
    ...
    for pair, score in zip(
        [p for p in test_pairs if p.category == "similar"], cosine_similar
    ):
        print(f"      {score:.4f} | {pair.description}")

    print(f"\n   Different topic pairs:")
    for pair, score in zip(
        [p for p in test_pairs if p.category == "different"], cosine_different
    ):
        print(f"      {score:.4f} | {pair.description}")

    print(f"\n   Identical pairs:")
    for pair, score in zip(
        [p for p in test_pairs if p.category == "identical"], cosine_identical
    ):
        print(f"      {score:.4f} | {pair.description}")
    ...
    print(f"   Similar pairs:")
    for pair, score in zip(
        [p for p in test_pairs if p.category == "similar"], bge_similar
    ):
        print(f"      {score:.4f} | {pair.description}")

    print(f"\n   Different topic pairs:")
    for pair, score in zip(
        [p for p in test_pairs if p.category == "different"], bge_different
    ):
        print(f"      {score:.4f} | {pair.description}")

    print(f"\n   Identical pairs:")
    for pair, score in zip(
        [p for p in test_pairs if p.category == "identical"], bge_identical
    ):
        print(f"      {score:.4f} | {pair.description}")
    ...
    print(
        f"   Cosine:  {cosine_separation:.4f} {'✅' if cosine_separation > 0 else '❌ OVERLAP'}"
    )
    print(
        f"   BGE:     {bge_separation:.4f} {'✅' if bge_separation > 0 else '❌ OVERLAP'}"
    )

    print(f"\n⏱️  Speed Comparison:")
    print(
        f"   Cosine:  {cosine_time:.1f}ms total, {cosine_time/len(test_pairs):.1f}ms/pair"
    )
    print(f"   BGE:     {bge_time:.1f}ms total, {bge_time/len(test_pairs):.1f}ms/pair")
    print(
        f"   Winner:  {'Cosine' if cosine_time < bge_time else 'BGE'} ({abs(cosine_time - bge_time):.1f}ms faster)"
    )
    ...
    for t in [0.95, 0.90, 0.85, 0.80, 0.75]:
        ...
        print(
            f"      {t:.2f}: {similar_hit}/{len(cosine_similar)} similar, {false_pos}/{len(cosine_different)} false positives"
        )
    ...
    for t in np.linspace(bge_min, bge_max, 6):
        ...
        print(
            f"      {t:.2f}: {similar_hit}/{len(bge_similar)} similar, {false_pos}/{len(bge_different)} false positives"
        )
