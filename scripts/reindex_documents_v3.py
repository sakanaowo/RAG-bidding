...
METADATA_FILE = Path(__file__).parent.parent / "data" / "processed" / "documents_metadata.json"
...
def load_documents_metadata() -> Dict[str, Any]:
    ...
    if isinstance(data, list):
        return {item.get("document_id", f"doc_{i}"): item for i, item in enumerate(data)}
    return data
...
def get_or_create_document(
    ...
    doc_name = metadata.get("title") or metadata.get("document_name") or doc_id.replace("_", " ").title()
...
def create_document_chunk(
    ...
    original_chunk_id = chunk_data.get("chunk_id", f"{document.document_id}_{chunk_index}")
    unique_chunk_id = f"{file_prefix}_{original_chunk_id}" if file_prefix else original_chunk_id
...
def verify_embedding_model():
    ...
    if "small" not in settings.embed_model.lower():
        logger.warning(f"⚠️ Expected text-embedding-3-small but got: {settings.embed_model}")
...
def reindex_documents(
    ...
    try:
        ...
        for chunk_file in chunk_files:
            ...
            document = get_or_create_document(db, f"{doc_id}_{file_prefix}", first_chunk, doc_metadata)
            ...
            for i, chunk_data in enumerate(chunks):
                ...
                doc_chunk = create_document_chunk(db, document, chunk_data, i, file_prefix)
        ...
        if not dry_run:
            # Verify counts
            embed_count = db.execute(text("SELECT COUNT(*) FROM langchain_pg_embedding")).scalar()
