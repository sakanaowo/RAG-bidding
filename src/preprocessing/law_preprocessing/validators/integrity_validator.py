...
class DataIntegrityValidator:
    ...
    def validate(
        ...
        checks = {
            ...
            "structure": self._check_structure_preservation(
                structure_tree, processed_chunks
            ),
        ...
        content_field = (
            "chunk_content"
            if processed_chunks and "chunk_content" in processed_chunks[0]
            else "content"
        )
    ...
    def _check_content_loss(self, original: str, chunks: List[Dict]) -> bool:
        ...
        if missing:
            missing_ratio = (
                len(missing) / len(original_markers) if original_markers else 0
            )
    ...
    def _check_structure_preservation(self, structure_tree, chunks: List[Dict]) -> bool:
        ...
        try:
            ...
            chunks_with_hierarchy = sum(
                1
                for chunk in chunks
                if (chunk.get("hierarchy_path") and chunk["hierarchy_path"].strip())
                or (chunk.get("hierarchy") and chunk["hierarchy"].strip())
            )
    ...
    def _find_missing_sections(self, original: str, chunks: List[Dict]) -> List[str]:
        ...
        return [
            f"Điều {d}"
            for d in sorted(missing, key=lambda x: int(re.search(r"\d+", x).group()))
        ]
