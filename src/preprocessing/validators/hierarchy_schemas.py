        ...
        for i, chunk in enumerate(chunks):
            ...
            if hierarchy and hierarchy.strip():
                ...
                if "Điều" not in hierarchy:
                    issues.append(
                        f"Chunk {i}: Invalid law hierarchy format '{hierarchy}'"
                    )
            else:
    ...
    def validate_hierarchy(self, chunks: List[Dict[str, Any]]) -> tuple[int, List[str]]:
        ...
        for i, chunk in enumerate(chunks):
            ...
            if hierarchy and hierarchy.strip():
                ...
                if "Điều" not in hierarchy:
                    issues.append(
                        f"Chunk {i}: Weak decree hierarchy '{hierarchy}' (no Điều reference)"
                    )
            else:
...
def validate_hierarchy_completeness(
    chunks: List[Dict[str, Any]], doc_type: Optional[str] = None
) -> tuple[int, List[str]]:
