...
ALLOWED_EXTENSIONS = set(settings.user_allowed_extensions)
...
async def upload_user_documents(
    files: List[UploadFile] = File(
        ..., description="Files to upload (max 3 files, 10MB each)"
    ),
    ...
    **Returns:** Upload ID to track status
    ...
    try:
        ...
        logger.info(
            f"📤 User {current_user.email} uploaded {len(files)} files: {result.get('upload_id')}"
        )
            ...
            result.append(
                {
                    ...
                    "status": row[1],
                    "file_count": (
                        len(files_data) if isinstance(files_data, list) else row[2]
                    ),
                    "filename": files_list[0]["filename"] if files_list else "Tài liệu",
                    "created_at": row[6].isoformat() if row[6] else None,
                    "user_note": (
                        admin_meta.get("user_note")
...
async def cancel_user_upload(
    ...
    try:
        ...
        if job.get("uploaded_by") != str(current_user.id):
            raise HTTPException(
                status_code=403, detail="Bạn không có quyền hủy upload này"
            )
