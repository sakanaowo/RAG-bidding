#!/usr/bin/env python3
"""
Test Main Circular Files Only

Test the circular preprocessing pipeline with only the main circular documents,
excluding appendix files that contain forms and templates.
"""
...
def test_main_circular_files():
    """Test circular preprocessing pipeline with main circular documents only"""
    
    # Configuration
    raw_data_dir = project_root / "data" / "raw" / "Thong tu"
    output_dir = project_root / "data" / "processed" / "circular_main_test"
    
    print("=" * 80)
    print("TESTING MAIN CIRCULAR DOCUMENTS ONLY")
    print("=" * 80)
    ...
    # Find all DOCX files
    all_docx_files = list(raw_data_dir.glob("*.docx"))
    
    # Filter to exclude appendix files (Phu luc)
    main_files = []
    excluded_files = []
    
    for docx_file in all_docx_files:
        filename_lower = docx_file.name.lower()
        if any(keyword in filename_lower for keyword in ['phu luc', 'phụ lục', 'appendix', 'biểu mẫu', 'mẫu']):
            excluded_files.append(docx_file)
        else:
            main_files.append(docx_file)
    
    print(f"✅ Found {len(all_docx_files)} total DOCX files")
    print(f"📄 Main circular files ({len(main_files)}):")
    for i, docx_file in enumerate(main_files, 1):
        size_mb = docx_file.stat().st_size / (1024 * 1024)
        print(f"  {i}. {docx_file.name} ({size_mb:.1f} MB)")
    
    if excluded_files:
        print(f"📋 Excluded appendix files ({len(excluded_files)}):")
        for docx_file in excluded_files:
            print(f"  - {docx_file.name} (appendix/forms)")
    
    if not main_files:
        print(f"❌ No main circular files found")
        return False
    ...
    for i, docx_file in enumerate(main_files, 1):
        print("=" * 60)
        print(f"PROCESSING FILE {i}/{len(main_files)}: {docx_file.name}")
        print("=" * 60)
        ...
        try:
            ...
            results_summary["statistics"]["coverage_scores"].append(file_result["coverage"])
    ...
    print("=" * 80)
    print("MAIN CIRCULAR FILES PROCESSING SUMMARY")
    print("=" * 80)
    
    print(f"📊 OVERALL RESULTS:")
    print(f"   Total main files: {results_summary['total_files']}")
    print(f"   Successful: {results_summary['successful']} ({results_summary['successful']/results_summary['total_files']*100:.1f}%)")
    print(f"   Failed: {results_summary['failed']} ({results_summary['failed']/results_summary['total_files']*100:.1f}%)")
    print(f"   With warnings: {results_summary['warnings']} ({results_summary['warnings']/results_summary['total_files']*100:.1f}%)")
    
    if results_summary["successful"] > 0:
        stats = results_summary["statistics"]
        stats["avg_chunk_size"] = stats["total_chars"] / stats["total_chunks"] if stats["total_chunks"] > 0 else 0
        avg_coverage = sum(stats["coverage_scores"]) / len(stats["coverage_scores"]) if stats["coverage_scores"] else 0
        
        print(f"\n📈 MAIN FILES STATISTICS:")
    ...
    for file_result in results_summary["files"]:
        status_icon = "✅" if file_result["status"] == "success" else "⚠️" if "warning" in file_result["status"] else "❌"
        print(f"   {status_icon} {file_result['filename']}")
        print(f"      Size: {file_result['file_size_mb']:.1f}MB | Chunks: {file_result['chunks']} | Coverage: {file_result['coverage']:.1f}% | Time: {file_result['processing_time']:.1f}s")
    ...
    report_file = output_dir / f"main_circular_processing_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    output_dir.mkdir(parents=True, exist_ok=True)
    
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("MAIN CIRCULAR PREPROCESSING REPORT\n")
        f.write("=" * 50 + "\n\n")
        f.write(f"Processed: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Total main files: {results_summary['total_files']}\n")
        f.write(f"Successful: {results_summary['successful']}\n")
        f.write(f"Failed: {results_summary['failed']}\n\n")
        
        f.write("MAIN FILE RESULTS:\n")
        f.write("-" * 30 + "\n")
        for file_result in results_summary["files"]:
...
if __name__ == "__main__":
    success = test_main_circular_files()
    
    if success:
        print(f"\n" + "=" * 80)
        print("✅ ALL MAIN CIRCULAR FILES PROCESSED SUCCESSFULLY!")
        print("=" * 80)
        sys.exit(0)
    else:
        print(f"\n" + "=" * 80)
        print("⚠️  SOME MAIN FILES FAILED - CHECK ERRORS ABOVE")
        print("=" * 80)
        sys.exit(1)